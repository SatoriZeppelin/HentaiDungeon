...
- 每块内：`<名称|种族>`（如 `<黑暗骑士|humanoid>`），以及 `<intent>…</intent>`。
- 意图行：`<target|scope|action|effect|param1|param2>`，用 `|` 分隔，空位保留。示例：`<player|single|attack||14|>`、`<player|aoe|multi_attack||4|3>`、`<self||taunt|||>`。
...
当前实现：**解析** → **`stats`**（enemyStats）→ **`commitSpawnPlanToBattle`** 写入聊天变量 **`enemyParty`** 并刷新战斗格子；敌方普攻/防御仍走 **battle.js** 既有逻辑（`atk`/`def`/`hp`）。
