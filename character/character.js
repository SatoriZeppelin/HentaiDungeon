/**
 * 人物数据（属性 + 被动/基础/可升级技能 + 特殊技能列表），集中维护。
 * 立绘由 resource/illustration.js 的 CHARACTER_PORTRAITS 提供，此处不写。
 * 每角色：specialSkills 为可解锁的特殊技能列表，specialSkillsUnlocked 为已解锁 id 数组。
 */
(function () {
  window.CHARACTERS = {
    达芙妮: {
      name: '达芙妮',
      from: '幻界',
      introduce: '战斗大师·近战输出',
      level: 5,
      sta: 10,
      exp: 0,
      str: 12,
      agi: 8,
      int: 3,
      def: 7,
      luk: 5,
      cha: 5,
      bonusStr: 0,
      bonusAgi: 0,
      bonusInt: 0,
      bonusSta: 0,
      bonusDef: 0,
      passiveSkills: [{ name: '狼族血脉', effect: '达芙妮获得 [等级 × 2] 的力量与防御加成。' }],
      skills: [
        {
          name: '攻击',
          ap: 1,
          effect: '对所选单位造成 [Str × 1] 点伤害',
          basic: true,
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '单体',
        },
        {
          name: '防御',
          ap: 1,
          effect: '对自身给予 [Def × 1] 点护盾，持续到自己的下个回合开始',
          basic: true,
          attribute1: '增益',
          attribute2: '自身',
          attribute3: '单体',
        },
        {
          name: '狼式旋风',
          ap: 2,
          level: 1,
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '群体',
          effectByLevel: [
            '挥舞巨剑横扫战场。对敌方全体造成 [Str × 0.9] 的物理伤害。',
            '挥舞巨剑横扫战场。对敌方全体造成 [Str × 1.0] 的物理伤害。',
            '挥舞巨剑横扫战场。对敌方全体造成 [Str × 1.1] 的物理伤害。',
            '挥舞巨剑横扫战场。对敌方全体造成 [Str × 1.2] 的物理伤害。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '血腥旋风',
              effect:
                '挥舞巨剑横扫战场。对敌方全体造成 [Str × 1.2] 的物理伤害。对所有命中目标施加 [Str × 0.2] 层【重伤】，如果暴击，额外施加 [Str × 0.2] 层【重伤】。',
            },
            {
              id: 'B',
              name: '重点打击',
              effect:
                '挥舞巨剑横扫战场。对敌方全体造成 [Str × 1.2] 的物理伤害。并对主要目标额外造成 [Str × 0.6] 的物理伤害。',
            },
          ],
        },
        {
          name: '剑脊格挡',
          ap: 1,
          level: 1,
          attribute1: '增益',
          attribute2: '自身',
          attribute3: '单体',
          effectByLevel: [
            '横置巨剑防御。获得 [Def × 0.5 + Str × 0.3] 的护盾。',
            '横置巨剑防御。获得 [Def × 0.6 + Str × 0.3] 的护盾。',
            '横置巨剑防御。获得 [Def × 0.7 + Str × 0.4] 的护盾。',
            '横置巨剑防御。获得 [Def × 0.8 + Str × 0.4] 的护盾。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '反击姿态',
              effect:
                '横置巨剑防御。获得 [Def × 0.8 + Str × 0.4] 的护盾。护盾被攻击破坏时，立即对攻击者进行反击，造成 [Str × 0.6] 的物理伤害。',
            },
            {
              id: 'B',
              name: '磐石之盾',
              effect: '横置巨剑防御。获得 [Def × 1.5] 的护盾。护盾存续期间，受到的伤害降低25%。',
            },
          ],
        },
        {
          name: '遒劲猛击',
          ap: 1,
          level: 1,
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '单体',
          effectByLevel: [
            '蓄力后挥出沉重一击。对单体造成 [Str × 1.05] 的物理伤害。',
            '蓄力后挥出沉重一击。对单体造成 [Str × 1.1] 的物理伤害。',
            '蓄力后挥出沉重一击。对单体造成 [Str × 1.15] 的物理伤害。',
            '蓄力后挥出沉重一击。对单体造成 [Str × 1.2] 的物理伤害。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '碎甲',
              effect: '蓄力后挥出沉重一击。对单体造成 [Str × 1.2] 的物理伤害，并施加1层【破甲】。',
            },
            {
              id: 'B',
              name: '撕裂',
              effect: '蓄力后挥出沉重一击。对单体造成 [Str × 1.2] 的物理伤害，并施加 [Str × 0.2] 层【重伤】。',
            },
          ],
        },
        {
          name: '斩杀',
          ap: 2,
          level: 1,
          locked: true,
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '单体',
          effectByLevel: [
            '尝试终结敌人。对单体造成 [Str × 1.3] 的物理伤害。如果目标生命值低于30%，造成2倍伤害。',
            '尝试终结敌人。对单体造成 [Str × 1.4] 的物理伤害。如果目标生命值低于30%，造成2倍伤害。',
            '尝试终结敌人。对单体造成 [Str × 1.5] 的物理伤害。如果目标生命值低于30%，造成2倍伤害。',
            '尝试终结敌人。对单体造成 [Str × 1.6] 的物理伤害。如果目标生命值低于30%，造成2倍伤害。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '乘胜追击',
              effect:
                '对单体造成 [Str × 1.6] 的物理伤害。如果目标生命值低于30%，造成2倍伤害。如果成功击杀敌人，释放一次50%效果的【狼式旋风】。',
            },
            {
              id: 'B',
              name: '龟裂创伤',
              effect: '尝试终结敌人。对单体造成 [Str × 1.6] 的物理伤害，施加 [Str × 0.4] 层【重伤】。',
            },
          ],
        },
      ],
      specialSkills: [
        {
          id: '残暴动力',
          name: '残暴动力',
          ap: 0,
          attribute1: '被动',
          attribute2: '',
          attribute3: '',
          effect: '每次【重伤】所造成的伤害会为达芙妮回复伤害量25%的血量。',
        },
        {
          id: '全副武装',
          name: '全副武装',
          ap: 0,
          attribute1: '被动',
          attribute2: '',
          attribute3: '',
          effect: '达芙妮获得 [Str × 0.25] 的额外防御属性（向下取整）。',
        },
        {
          id: '威慑怒吼',
          name: '威慑怒吼',
          ap: 2,
          attribute1: '增益',
          attribute2: '自身',
          attribute3: '群体',
          effect:
            '发出震慑敌人的怒吼，获得2层【嘲讽】，并获得 [Def × 1.2] 的护盾，每存在一个敌人，额外获得 [Def × 0.4] 的护盾。',
        },
        {
          id: '狼群围猎',
          name: '狼群围猎',
          ap: 2,
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '单体',
          effect:
            '与白牙协同攻击。对单体造成 [Str × 1.5] 的物理伤害。若【白牙】存活，白牙立即对同一目标发动一次免费的【撕咬】。若【白牙】不存在，改为施加2次【重伤】。',
        },
        {
          id: '白牙！',
          name: '白牙！',
          ap: 2,
          attribute1: '召唤',
          attribute2: '',
          attribute3: '',
          effect: '召唤一头巨狼【白牙】，持续至战斗结束。',
        },
      ],
      specialSkillsUnlocked: [],
    },
    普罗安妲: {
      name: '普罗安妲',
      level: 5,
      sta: 8,
      exp: 0,
      str: 2,
      agi: 8,
      int: 20,
      def: 4,
      luk: 8,
      cha: 6,
      passiveSkills: [],
      skills: [
        {
          name: '攻击',
          ap: 1,
          effect: '对所选单位造成 [Str × 1] 点伤害',
          basic: true,
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '单体',
        },
        {
          name: '防御',
          ap: 1,
          effect: '对自身给予 [Def × 1] 点护盾，持续到自己的下个回合开始',
          basic: true,
          attribute1: '增益',
          attribute2: '自身',
          attribute3: '单体',
        },
      ],
      specialSkills: [],
      specialSkillsUnlocked: [],
    },
    '丝伊德·白': {
      name: '丝伊德·白',
      from: '幻界',
      introduce: '待填写',
      level: 5,
      sta: 14,
      exp: 0,
      str: 8,
      agi: 6,
      int: 10,
      def: 6,
      luk: 6,
      cha: 8,
      bonusStr: 0,
      bonusAgi: 0,
      bonusInt: 0,
      bonusSta: 0,
      bonusDef: 0,
      bonusLuk: 0,
      bonusCha: 0,
      passiveSkills: [
        {
          name: '共生母胎',
          effect: '每次受到伤害时获得1层【孕育】。场上每存活1个女儿单位，耐力+3。',
        },
      ],
      skills: [
        {
          name: '攻击',
          ap: 1,
          effect: '对所选单位造成 [Str × 1] 点伤害',
          basic: true,
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '单体',
        },
        {
          name: '防御',
          ap: 1,
          effect: '对自身给予 [Def × 1] 点护盾，持续到自己的下个回合开始',
          basic: true,
          attribute1: '增益',
          attribute2: '自身',
          attribute3: '单体',
        },
        {
          name: '碧血魔剑',
          ap: 1,
          level: 5,
          advancement: 'A',
          attribute1: '混合',
          attribute2: '近战',
          attribute3: '单体',
          effectByLevel: [
            '以共生之血凝成魔剑斩向敌人。对单体造成 [Str × 0.4] 的物理伤害和 [Int × 0.4] 的自然伤害。',
            '以共生之血凝成魔剑斩向敌人。对单体造成 [Str × 0.5] 的物理伤害和 [Int × 0.4] 的自然伤害。',
            '以共生之血凝成魔剑斩向敌人。对单体造成 [Str × 0.5] 的物理伤害和 [Int × 0.5] 的自然伤害。',
            '以共生之血凝成魔剑斩向敌人。对单体造成 [Str × 0.6] 的物理伤害和 [Int × 0.6] 的自然伤害。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '侵蚀共鸣',
              effect:
                '以共生之血凝成魔剑斩向敌人。对单体造成 [Str × 0.6] 的物理伤害和 [Int × 0.6] 的自然伤害。若【可瑞】存活，可瑞立即对同一目标施加2层【破甲】。',
            },
            {
              id: 'B',
              name: '粘液浸透',
              effect:
                '以共生之血凝成魔剑斩向敌人。对单体造成 [Str × 0.6] 的物理伤害和 [Int × 0.6] 的自然伤害。若【莉莉姆】存活，莉莉姆对目标施加1层【迟缓】，莉莉姆和丝伊德回复 [莉莉姆DEF × 0.3] 的生命值。',
            },
          ],
        },
        {
          name: '威吓',
          ap: 1,
          level: 5,
          advancement: 'A',
          attribute1: '增益',
          attribute2: '自身',
          attribute3: '群体',
          effectByLevel: [
            '施加2层【嘲讽】。丝伊德·白获得 [Def × 0.5] 的护盾。',
            '施加2层【嘲讽】。丝伊德·白获得 [Def × 0.6] 的护盾。',
            '施加2层【嘲讽】。丝伊德·白获得 [Def × 0.7] 的护盾。',
            '施加2层【嘲讽】。丝伊德·白获得 [Def × 0.8] 的护盾。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '菌丝摇篮',
              effect:
                '施加2层【嘲讽】。丝伊德·白获得 [Def × 0.8] 的护盾。若【堇】存活，堇立即对丝伊德·白发动一次免费的【再生菌丝】。',
            },
            {
              id: 'B',
              name: '金粉帷幕',
              effect:
                '施加2层【嘲讽】。丝伊德·白获得 [Def × 0.8] 的护盾。若【蒙特卡洛】存活，蒙特卡洛立即发动一次免费的【金粉弥漫】。',
            },
          ],
        },
        {
          name: '绯色轮舞',
          ap: 2,
          level: 5,
          advancement: 'A',
          attribute1: '混合',
          attribute2: '近战',
          attribute3: '群体',
          effectByLevel: [
            '对敌方全体造成 [Str × 0.4] 的物理伤害和 [Int × 0.4] 的自然伤害。',
            '对敌方全体造成 [Str × 0.5] 的物理伤害和 [Int × 0.4] 的自然伤害。',
            '对敌方全体造成 [Str × 0.5] 的物理伤害和 [Int × 0.5] 的自然伤害。',
            '对敌方全体造成 [Str × 0.6] 的物理伤害和 [Int × 0.6] 的自然伤害。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '孢子旋风',
              effect:
                '对敌方全体造成 [Str × 0.6] 的物理伤害和 [Int × 0.6] 的自然伤害。若【蒙特卡洛】存活，额外对所有命中目标施加1层【恍惚】和1层【虚弱】。',
            },
            {
              id: 'B',
              name: '血触风暴',
              effect:
                '对敌方全体造成 [Str × 0.6] 的物理伤害和 [Int × 0.6] 的自然伤害。若【可瑞】存活，对所有命中目标额外施加1层【破甲】，可瑞立即对随机敌方单体发动一次免费的【血触侵蚀】。',
            },
          ],
        },
        {
          name: '魔物孕育',
          ap: 2,
          level: 5,
          advancement: 'A',
          attribute1: '召唤',
          attribute2: '自身',
          attribute3: '单体',
          effectByLevel: [
            '消耗8层【孕育】，选择1个己方空位，随机召唤1个女儿（莉莉姆、可瑞、蒙特卡洛、堇之一）。',
            '消耗7层【孕育】，选择1个己方空位，随机召唤1个女儿（莉莉姆、可瑞、蒙特卡洛、堇之一）。',
            '消耗6层【孕育】，选择1个己方空位，随机召唤1个女儿（莉莉姆、可瑞、蒙特卡洛、堇之一）。',
            '消耗5层【孕育】，选择1个己方空位，随机召唤1个女儿（莉莉姆、可瑞、蒙特卡洛、堇之一）。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '孕育加速',
              effect: '回合开始时，丝伊德·白获得1层【孕育】。消耗5层【孕育】，选择1个己方空位，随机召唤1个女儿。',
            },
            {
              id: 'B',
              name: '双生孕育',
              effect: '消耗8层【孕育】，依次选择2个不同的己方空位，随机召唤2个不同的女儿。两个女儿的 MaxHP 各 -20%。',
            },
          ],
        },
      ],
      specialSkills: [
        {
          id: '姬骑解禁',
          name: '全武装形态·姬骑解禁',
          ap: 3,
          attribute1: '增益',
          attribute2: '自身',
          attribute3: '单体',
          effect:
            '进入【姬骑】形态至战斗结束：全属性+50%，免疫控制，AP上限-2；每回合开始自动对随机敌方发动一次无消耗【碧血魔剑】。',
        },
        {
          id: '枝叶硕茂',
          name: '枝叶硕茂',
          ap: 0,
          attribute1: '被动',
          attribute2: '',
          attribute3: '',
          effect: '每层【孕育】额外+1力量与+1智力；若【蒙特卡洛】存活则改为每层+2力量与+2智力。',
        },
        {
          id: '腐蚀领域',
          name: '腐蚀领域',
          ap: 2,
          attribute1: '自然',
          attribute2: '远程',
          attribute3: '群体',
          effect:
            '对敌方全体造成 [Int × 1.2] 的自然伤害，施加1次【中毒】。场上每有1个处于中毒状态的敌人，丝伊德·白获得1层【孕育】。若【堇】存活，本次【腐蚀领域】施加的中毒层数+50%（向下取整）。',
        },
        {
          id: '异种外壳',
          name: '异种外壳',
          ap: 2,
          attribute1: '增益',
          attribute2: '自身',
          attribute3: '单体',
          effect:
            '获得 [Sta × 1.2 + Def × 1.6] 护盾；至多消耗2层【孕育】，每层清除1个负面。若【莉莉姆】存活则护盾改为莉莉姆当前生命×1.2。每20点护盾获得1层【孕育】，至多3层。',
        },
        {
          id: '破阵冲锋',
          name: '破阵冲锋',
          ap: 2,
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '单体',
          effect:
            '造成 [Str × 1.5 + Def × 0.75] 物理伤害并1层【破甲】。若【可瑞】存活，可瑞对全体敌人各发动50%伤害的【缠绕撕咬】并1层【破甲】。',
        },
      ],
      specialSkillsUnlocked: [],
      buffs: [{ id: '孕育', name: '孕育', layers: 10 }],
    },
    艾丽卡: {
      name: '艾丽卡',
      from: '幻界',
      introduce: '权天使·治疗坦克',
      level: 5,
      sta: 12,
      exp: 0,
      str: 8,
      agi: 6,
      int: 15,
      def: 8,
      luk: 5,
      cha: 7,
      passiveSkills: [
        {
          name: '神恩之躯',
          effect: '艾丽卡获得 [Int × 0.25] 的额外耐力属性和 [Int × 0.25] 的额外防御属性。',
        },
      ],
      skills: [
        {
          name: '攻击',
          ap: 1,
          effect: '对所选单位造成 [Str × 1] 点伤害',
          basic: true,
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '单体',
        },
        {
          name: '防御',
          ap: 1,
          effect: '对自身给予 [Def × 1] 点护盾，持续到自己的下个回合开始',
          basic: true,
          attribute1: '增益',
          attribute2: '自身',
          attribute3: '单体',
        },
        {
          name: '圣光斩',
          ap: 1,
          level: 1,
          attribute1: '混合',
          attribute2: '近战',
          attribute3: '单体',
          effectByLevel: [
            '挥舞圣剑进行神圣斩击。对单体造成 [Str × 0.5] 的物理伤害和 [Int × 0.4] 的神圣伤害。',
            '挥舞圣剑进行神圣斩击。对单体造成 [Str × 0.6] 的物理伤害和 [Int × 0.4] 的神圣伤害。',
            '挥舞圣剑进行神圣斩击。对单体造成 [Str × 0.6] 的物理伤害和 [Int × 0.5] 的神圣伤害。',
            '挥舞圣剑进行神圣斩击。对单体造成 [Str × 0.7] 的物理伤害和 [Int × 0.5] 的神圣伤害。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '炽天之剑',
              effect:
                '挥舞圣剑进行神圣斩击。对单体造成 [Str × 0.7] 的物理伤害和 [Int × 0.5] 的神圣伤害。施加2次【燃烧】。',
            },
            {
              id: 'B',
              name: '净化斩击',
              effect:
                '挥舞圣剑进行神圣斩击。对单体造成 [Str × 0.7] 的物理伤害和 [Int × 0.5] 的神圣伤害。命中时驱散目标1个增益Buff。若成功驱散，艾丽卡回复 [Int × 0.4] 的生命值。',
            },
          ],
        },
        {
          name: '清算之手',
          ap: 1,
          level: 1,
          attribute1: '神圣',
          attribute2: '远程',
          attribute3: '单体',
          effectByLevel: [
            '以神圣之力标记罪人。对单体造成 [Int × 0.4] 的神圣伤害，艾丽卡获得2层【嘲讽】。',
            '以神圣之力标记罪人。对单体造成 [Int × 0.45] 的神圣伤害，艾丽卡获得2层【嘲讽】。',
            '以神圣之力标记罪人。对单体造成 [Int × 0.5] 的神圣伤害，艾丽卡获得2层【嘲讽】。',
            '以神圣之力标记罪人。对单体造成 [Int × 0.6] 的神圣伤害，艾丽卡获得2层【嘲讽】。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '制裁',
              effect: '以神圣之力标记罪人。对单体造成 [Int × 0.6] 的神圣伤害，施加1层【眩晕】，艾丽卡获得2层【嘲讽】。',
            },
            {
              id: 'B',
              name: '罪印',
              effect:
                '以神圣之力标记罪人。对单体造成 [Int × 0.6] 的神圣伤害，施加1层【碎魔】和1层【破甲】，艾丽卡获得2层【嘲讽】。',
            },
          ],
        },
        {
          name: '神恩救赎',
          ap: 1,
          level: 1,
          attribute1: '治疗',
          attribute2: '远程',
          attribute3: '单体',
          effectByLevel: [
            '召唤圣光瞬间治愈目标。为友方单体回复 [Int × 0.7] 的生命值。',
            '召唤圣光瞬间治愈目标。为友方单体回复 [Int × 0.8] 的生命值。',
            '召唤圣光瞬间治愈目标。为友方单体回复 [Int × 0.9] 的生命值。',
            '召唤圣光瞬间治愈目标。为友方单体回复 [Int × 1.0] 的生命值。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '自愈圣光',
              effect: '为友方单体回复 [Int × 1.0] 的生命值。若目标为艾丽卡自身，治疗量额外+50%。',
            },
            {
              id: 'B',
              name: '厚泽',
              effect: '为友方单体回复 [Int × 1.0 + Sta × 0.8] 的生命值。',
            },
          ],
        },
        {
          name: '罪罚宣告',
          ap: 2,
          level: 1,
          attribute1: '神圣',
          attribute2: '远程',
          attribute3: '群体',
          effectByLevel: [
            '以神圣权柄宣告罪罚。对敌方全体造成 [Int × 0.8] 的神圣伤害，施加1层【虚弱】。',
            '以神圣权柄宣告罪罚。对敌方全体造成 [Int × 0.9] 的神圣伤害，施加1层【虚弱】。',
            '以神圣权柄宣告罪罚。对敌方全体造成 [Int × 1.0] 的神圣伤害，施加1层【虚弱】。',
            '以神圣权柄宣告罪罚。对敌方全体造成 [Int × 1.1] 的神圣伤害，施加1层【虚弱】。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '血祭宣判',
              effect:
                '以神圣权柄宣告罪罚。消耗自身20%当前HP，对敌方全体造成 [Int × 1.1 + Sta × 0.6] 的神圣伤害。对所有命中目标施加1层【虚弱】。',
            },
            {
              id: 'B',
              name: '圣言镇压',
              effect: '对敌方全体造成 [Int × 1.1] 的神圣伤害。对所有命中目标施加1层【虚弱】和2层【脆弱】。',
            },
          ],
        },
      ],
      specialSkills: [
        {
          id: '盲目之光',
          name: '盲目之光',
          ap: 1,
          attribute1: '减益',
          attribute2: '远程',
          attribute3: '群体',
          effect: '释放刺目的神圣光芒灼烧敌人视觉。对所有敌人施加2层【恍惚】和2层【迟钝】。',
        },
        {
          id: '奉献',
          name: '奉献',
          ap: 0,
          attribute1: '被动',
          attribute2: '',
          attribute3: '',
          effect: '艾丽卡的回合开始时，对自身和敌方全体造成 [Sta × 1.0] 的神圣伤害。',
        },
        {
          id: '殉道圣躯',
          name: '殉道圣躯',
          ap: 0,
          attribute1: '被动',
          attribute2: '',
          attribute3: '',
          effect:
            '艾丽卡每在自己的回合受到一次伤害，获得1层【智力强化】，每在敌人的回合受到一次伤害，获得1层【防御强化】',
        },
        {
          id: '救赎',
          name: '救赎',
          ap: 3,
          attribute1: '治疗',
          attribute2: '远程',
          attribute3: '单体',
          effect:
            '消耗自身30%当前HP，回复150%消耗量的生命值，清除所有负面状态。若目标HP未回复至满，额外获得 [Sta × 1.5] 的护盾。',
        },
        {
          id: '圣火净世',
          name: '圣火净世',
          ap: 2,
          attribute1: '神圣',
          attribute2: '远程',
          attribute3: '群体',
          effect:
            '引爆自身神圣之力席卷战场。对敌方全体造成 [Int × 0.8] 的神圣伤害，对自身和所有命中目标施加2次【燃烧】。艾丽卡承受 [Sta × 0.5] 的自伤。每命中1个敌人，艾丽卡获得 [Def × 0.4] 的护盾。',
        },
      ],
      specialSkillsUnlocked: ['盲目之光', '奉献', '殉道圣躯', '救赎', '圣火净世'],
    },
    黯: {
      name: '黯',
      from: '幻界',
      introduce: '魔枪舞者·连击输出',
      level: 5,
      sta: 6,
      exp: 0,
      str: 8,
      agi: 14,
      int: 8,
      def: 2,
      luk: 5,
      cha: 9,
      bonusStr: 0,
      bonusAgi: 0,
      bonusInt: 0,
      bonusSta: 0,
      bonusDef: 0,
      passiveSkills: [
        {
          name: '暗黑魔枪术',
          effect:
            '黯造成的任何伤害都会额外附带 [Int × 0.4] 的暗影属性魔法伤害。\n黯获得 [Agi × 0.2] 的智力，获得 [Int × 0.2] 的敏捷。',
        },
      ],
      skills: [
        {
          name: '攻击',
          ap: 1,
          effect: '对所选单位造成 [Str × 1] 点伤害',
          basic: true,
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '单体',
        },
        {
          name: '防御',
          ap: 1,
          effect: '对自身给予 [Def × 1] 点护盾，持续到自己的下个回合开始',
          basic: true,
          attribute1: '增益',
          attribute2: '自身',
          attribute3: '单体',
        },
        {
          name: '幽灵舞踏',
          ap: 2,
          level: 1,
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '单体',
          effectByLevel: [
            '踏入残影进行连续突刺。对单体进行3次攻击判定，每次造成 [Str × 0.35] 的物理伤害。',
            '踏入残影进行连续突刺。对单体进行3次攻击判定，每次造成 [Str × 0.4] 的物理伤害。',
            '踏入残影进行连续突刺。对单体进行3次攻击判定，每次造成 [Str × 0.45] 的物理伤害。',
            '踏入残影进行连续突刺。对单体进行3次攻击判定，每次造成 [Str × 0.5] 的物理伤害。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '疾风',
              effect: '对单体进行4次攻击判定，每次造成 [Str × 0.5] 的物理伤害，首次命中时，施加1层【流血】。',
            },
            {
              id: 'B',
              name: '蚀魂',
              effect: '对单体进行3次攻击判定，每次造成 [Str × 0.5] 的物理伤害。每次命中施加1层【暗蚀】。',
            },
          ],
        },
        {
          name: '残影步',
          ap: 1,
          level: 1,
          attribute1: '增益',
          attribute2: '自身',
          attribute3: '单体',
          effectByLevel: [
            '黯获得2层【灵巧】，持续至下回合。成功闪避时，对攻击者造成 [Str × 0.3] 的物理伤害。',
            '黯获得2层【灵巧】，持续至下回合。成功闪避时，对攻击者造成 [Str × 0.4] 的物理伤害。',
            '黯获得3层【灵巧】，持续至下回合。成功闪避时，对攻击者造成 [Str × 0.4] 的物理伤害。',
            '黯获得3层【灵巧】，持续至下回合。成功闪避时，对攻击者造成 [Str × 0.5] 的物理伤害。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '血影',
              effect:
                '黯获得3层【灵巧】，持续至下回合。成功闪避时，对攻击者造成 [Str × 0.5] 的物理伤害并施加1次【流血】。',
            },
            {
              id: 'B',
              name: '暗袭',
              effect:
                '黯获得3层【灵巧】，持续至下回合。成功闪避时，对攻击者造成 [Str × 0.2 + Int × 0.3] 的混合伤害并施加1层【暗蚀】。',
            },
          ],
        },
        {
          name: '血舞枪刃',
          ap: 1,
          level: 1,
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '单体',
          effectByLevel: [
            '以枪尖划开伤口。对单体造成 [Str × 0.5 + Agi × 0.3] 的物理伤害，并施加1次【流血】。',
            '以枪尖划开伤口。对单体造成 [Str × 0.6 + Agi × 0.3] 的物理伤害，并施加1次【流血】。',
            '以枪尖划开伤口。对单体造成 [Str × 0.7 + Agi × 0.4] 的物理伤害，并施加1次【流血】。',
            '以枪尖划开伤口。对单体造成 [Str × 0.8 + Agi × 0.4] 的物理伤害，并施加1次【流血】。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '撕裂',
              effect:
                '对单体造成 [Str × 0.8 + Agi × 0.4] 的物理伤害，施加1次【流血】。目标每有10层【流血】，此技能伤害+20%（最高+60%）。',
            },
            {
              id: 'B',
              name: '暗影枪刃',
              effect: '对单体造成 [Str × 0.8 + Int × 0.4] 的魔法伤害，施加1次【流血】和1层【暗蚀】。',
            },
          ],
        },
        {
          name: '暗夜帷幕',
          ap: 1,
          level: 1,
          locked: true,
          attribute1: '暗影',
          attribute2: '远程',
          attribute3: '群体',
          effectByLevel: [
            '释放暗属性雾气笼罩战场。对敌方全体造成 [Int × 0.45] 的魔法伤害。',
            '释放暗属性雾气笼罩战场。对敌方全体造成 [Int × 0.5] 的魔法伤害。',
            '释放暗属性雾气笼罩战场。对敌方全体造成 [Int × 0.55] 的魔法伤害。',
            '释放暗属性雾气笼罩战场。对敌方全体造成 [Int × 0.6] 的魔法伤害。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '窒息迷雾',
              effect:
                '对敌方全体造成 [Int × 0.6] 的魔法伤害，施加2层【虚弱】。2回合内，每回合结束时敌方全体受到 [Int × 0.25] 的魔法伤害。',
            },
            {
              id: 'B',
              name: '蚀影蔓延',
              effect: '对敌方全体造成 [Int × 0.6] 的魔法伤害，对所有命中目标施加2层【暗蚀】。',
            },
          ],
        },
      ],
      specialSkills: [
        {
          id: '魔龙舞',
          name: '魔龙舞',
          ap: 3,
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '单体',
          effect: '化作无数残影进行超高速连击。对单体目标进行3+[Agi×0.125]次判定，每次造成 [Agi × 0.4] 的物理伤害。',
        },
        {
          id: '深渊终结',
          name: '深渊终结',
          ap: 2,
          attribute1: '暗影',
          attribute2: '近战',
          attribute3: '单体',
          effect:
            '集中暗属性魔力进行贯穿。对单体造成 [Int × 2.0] 的魔法伤害。目标身上每有1层【暗蚀】，伤害额外+20%（最高+60%）。施放后消耗目标所有【暗蚀】层数。',
        },
        {
          id: '暗蚀之刃',
          name: '暗蚀之刃',
          ap: 1,
          attribute1: '暗影',
          attribute2: '近战',
          attribute3: '单体',
          effect: '对单体造成 [Int × 0.8] 的魔法伤害，施加2层【暗蚀】。若目标已有【暗蚀】，额外施加1层【碎魔】。',
        },
        {
          id: '暗影渗透',
          name: '暗影渗透',
          ap: 0,
          attribute1: '被动',
          attribute2: '',
          attribute3: '',
          effect: '黯对拥有【暗蚀】的目标施加【流血】时，层数+50%。黯对拥有【流血】的目标施加【暗蚀】时，额外施加1层。',
        },
        {
          id: '错锋',
          name: '错锋',
          ap: 0,
          attribute1: '被动',
          attribute2: '',
          attribute3: '',
          effect: '黯的基础指令【攻击】改为进行3次攻击判定，每次造成 [Str × 0.3] 的物理伤害。',
        },
      ],
      specialSkillsUnlocked: [],
    },
    昼墨: {
      name: '昼墨',
      from: '幻界',
      introduce: '醉太懒狗还没写',
      level: 5,
      sta: 10,
      exp: 0,
      str: 10,
      agi: 12,
      int: 6,
      def: 8,
      luk: 4,
      cha: 6,
      passiveSkills: [{ name: '刀势流转', effect: '启用【攻势】和【守势】机制。' }],
      skills: [
        {
          name: '攻击',
          ap: 1,
          effect: '对所选单位造成 [Str × 1] 点伤害',
          basic: true,
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '单体',
        },
        {
          name: '防御',
          ap: 1,
          effect: '对自身给予 [Def × 1] 点护盾，持续到自己的下个回合开始',
          basic: true,
          attribute1: '增益',
          attribute2: '自身',
          attribute3: '单体',
        },
        {
          name: '斩月',
          ap: 1,
          level: 1,
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '单体',
          effectByLevel: [
            '迅捷的横斩。对单体造成 [Str × 0.9] 的物理伤害。获得2层【攻势】。',
            '迅捷的横斩。对单体造成 [Str × 1.0] 的物理伤害。获得2层【攻势】。',
            '迅捷的横斩。对单体造成 [Str × 1.1] 的物理伤害。获得2层【攻势】。',
            '迅捷的横斩。对单体造成 [Str × 1.2] 的物理伤害。获得2层【攻势】。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '疾风斩月',
              effect:
                '迅捷的横斩。对单体造成 [Str × 1.2] 的物理伤害。获得3层【攻势】。若本回合已拥有【守势】，改为造成 [Str × 1.5] 的物理伤害。',
            },
            {
              id: 'B',
              name: '残月',
              effect:
                '迅捷的横斩。对单体造成 [Str × 1.2] 的物理伤害。命中后施加1次【流血】。若【攻势】≥5层，额外施加1层【破甲】。',
            },
          ],
        },
        {
          name: '见切',
          ap: 1,
          level: 1,
          attribute1: '增益',
          attribute2: '自身',
          attribute3: '单体',
          effectByLevel: [
            '收刀蓄势，进入见切姿态。获得2层【守势】，并获得 [Def × 0.9] 的护盾。',
            '收刀蓄势，进入见切姿态。获得2层【守势】，并获得 [Def × 1.0] 的护盾。',
            '收刀蓄势，进入见切姿态。获得2层【守势】，并获得 [Def × 1.1] 的护盾。',
            '收刀蓄势，进入见切姿态。获得2层【守势】，并获得 [Def × 1.2] 的护盾。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '弹返',
              effect:
                '获得2层【守势】，并获得 [Def × 1.2] 的护盾。本回合内护盾被攻击破坏时，立即对攻击者进行反击，造成 [Str × 0.6] 的物理伤害。',
            },
            {
              id: 'B',
              name: '残心',
              effect: '获得3层【守势】，并获得 [Def × 1.2] 的护盾。和1层【坚韧】。',
            },
          ],
        },
        {
          name: '居合',
          ap: 1,
          level: 1,
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '单体',
          effectByLevel: [
            '拔刀斩击。对单体造成 [Str × 0.6 + Agi × 0.3] 的物理伤害。获得1层【攻势】和1层【守势】。',
            '拔刀斩击。对单体造成 [Str × 0.7 + Agi × 0.3] 的物理伤害。获得1层【攻势】和1层【守势】。',
            '拔刀斩击。对单体造成 [Str × 0.7 + Agi × 0.4] 的物理伤害。获得1层【攻势】和1层【守势】。',
            '拔刀斩击。对单体造成 [Str × 0.8 + Agi × 0.4] 的物理伤害。获得1层【攻势】和1层【守势】。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '破',
              effect: '拔刀斩击。对单体造成 [Str × 1.0 + Agi × 0.5]。若【攻势】>【守势】，额外获得1层【攻势】。',
            },
            {
              id: 'B',
              name: '守',
              effect: '攻击后获得 [Def × 0.5] 的护盾。若【守势】>【攻势】，额外获得1层【守势】。',
            },
          ],
        },
        {
          name: '纳刀',
          ap: 1,
          level: 1,
          locked: true,
          attribute1: '增益',
          attribute2: '自身',
          attribute3: '单体',
          effectByLevel: [
            '调整呼吸，转化刀势。\n若【攻势】>【守势】：消耗所有【守势】转化为等量【攻势】，下一次攻击伤害提升[转化层数×5%]。\n若【守势】>【攻势】：消耗所有【攻势】转化为等量【守势】，获得[Def×0.1×转化层数]护盾。\n若【攻势】=【守势】：获得1层【攻势】和1层【守势】。',
            '调整呼吸，转化刀势。\n若【攻势】>【守势】：消耗所有【守势】转化为等量【攻势】，下一次攻击伤害提升[转化层数×5%]。\n若【守势】>【攻势】：消耗所有【攻势】转化为等量【守势】，获得[Def×0.15×转化层数]护盾。\n若【攻势】=【守势】：获得1层【攻势】和1层【守势】。',
            '调整呼吸，转化刀势。\n若【攻势】>【守势】：消耗所有【守势】转化为等量【攻势】，下一次攻击伤害提升[转化层数×7.5%]。\n若【守势】>【攻势】：消耗所有【攻势】转化为等量【守势】，获得[Def×0.15×转化层数]护盾。\n若【攻势】=【守势】：获得1层【攻势】和1层【守势】。',
            '调整呼吸，转化刀势。\n若【攻势】>【守势】：消耗所有【守势】转化为等量【攻势】，下一次攻击伤害提升[转化层数×10%]。\n若【守势】>【攻势】：消耗所有【攻势】转化为等量【守势】，获得[Def×0.2×转化层数]护盾。\n若【攻势】=【守势】：获得1层【攻势】和1层【守势】。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '归鞘',
              effect:
                '若【攻势】>【守势】：消耗所有【守势】转化为等量【攻势】，下一次攻击伤害提升[转化层数×10%]；若转化后【攻势】＞8层，获得1层【力量强化】持续3回合。\n若【守势】>【攻势】：消耗所有【攻势】转化为等量【守势】，获得[Def×0.2×转化层数]护盾；若转化后【守势】＞8层，获得1层【防御强化】持续3回合。\n若【攻势】=【守势】：获得1层【攻势】和1层【守势】。',
            },
            {
              id: 'B',
              name: '共鸣',
              effect:
                '若【攻势】>【守势】：消耗所有【守势】转化为等量【攻势】，下一次攻击伤害提升[转化层数×10%]。\n若【守势】>【攻势】：消耗所有【攻势】转化为等量【守势】，获得[Def×0.2×转化层数]护盾。\n若【攻势】=【守势】：获得1层【攻势】和1层【守势】；下一次攻击获得总架势×5%的暴击伤害加成。',
            },
          ],
        },
      ],
      specialSkills: [
        {
          id: '错金',
          name: '错金',
          ap: 2,
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '单体',
          effect:
            '对单体造成 [Str × 1.6] 的物理伤害。\n若【攻势】>【守势】：对目标施加1层【破甲】（3回合）。\n若【守势】>【攻势】：对目标施加1层【缴械】。\n若【攻势】=【守势】：同时施加两种效果。',
        },
        {
          id: '白夜',
          name: '白夜',
          ap: 2,
          attribute1: '物理',
          attribute2: '远程',
          attribute3: '群体',
          effect:
            '刀身泛起惨白光芒横斩。对敌方全体造成 [Str × 0.5 + Agi × 0.5] 的物理伤害。\n若【攻势】≥10层：斩击汇聚为一点，改为对当前选定目标造成 [Str × 1.2 + Agi × 1.2] 的物理伤害，并施加2层【破甲】。\n若【守势】≥10层：对敌方全体造成 [Str × 0.75 + Agi × 0.75] 的物理伤害，若暴击，施加1层【眩晕】。\n若【攻势】和【守势】均≥10层：对敌方全体造成 [Str × 1.2 + Agi × 1.2] 的物理伤害，且对命中的所有敌人施加一次[幸运×5%]即死判定；未即死则造成正常伤害。',
        },
        {
          id: '一闪',
          name: '一闪',
          ap: 3,
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '单体',
          effect:
            '对单体造成 [Str × 3.0] 的物理伤害。\n若【攻势】恰好为5层：伤害增加20%。\n若【守势】恰好为5层：获得 [Def × 1.5] 的护盾。\n若【攻势】=【守势】且均为5层：伤害+50%，获得 [Def × 2.5] 的护盾，此次攻击必定命中。',
        },
        {
          id: '心眼',
          name: '心眼',
          ap: 0,
          attribute1: '被动',
          attribute2: '',
          attribute3: '',
          effect: '闪避率增加[守势层数×5%]，暴击率增加[攻势层数×5%]，回合开始时获得2层【攻势】和2层【守势】。',
        },
        {
          id: '无拍子',
          name: '无拍子',
          ap: 1,
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '单体',
          effect:
            '瞬间拔刀斩击。对单体造成 [Agi × 1.2] 的物理伤害，此次攻击必定命中。\n若本回合未获得过任何【攻势】或【守势】：伤害+50%，并获得3层【攻势】和3层【守势】。',
        },
      ],
      specialSkillsUnlocked: [],
      buffs: [],
    },
    夜露: {
      name: '夜露',
      from: '幻界',
      introduce: '魅魔术师·魔法输出',
      level: 5,
      sta: 6,
      exp: 0,
      str: 4,
      agi: 10,
      int: 16,
      def: 2,
      luk: 4,
      cha: 10,
      passiveSkills: [
        {
          name: '魔力渴求',
          effect:
            '夜露对处于【魅惑】、【混乱】或【眩晕】状态的敌人造成的伤害+20%。若夜露成功施加1次【魅惑】，回复 [Cha × 0.5] 的生命值并获得1层【智力强化】。',
        },
      ],
      skills: [
        {
          name: '攻击',
          ap: 1,
          effect: '对所选单位造成 [Str × 1] 点伤害',
          basic: true,
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '单体',
        },
        {
          name: '防御',
          ap: 1,
          effect: '对自身给予 [Def × 1] 点护盾，持续到自己的下个回合开始',
          basic: true,
          attribute1: '增益',
          attribute2: '自身',
          attribute3: '单体',
        },
        {
          name: '炎魔吹息',
          ap: 1,
          level: 1,
          attribute1: '火焰',
          attribute2: '远程',
          attribute3: '单体',
          effectByLevel: [
            '对单体造成 [Int × 0.8] 的火焰伤害，施加1次【燃烧】。',
            '对单体造成 [Int × 0.9] 的火焰伤害，施加1次【燃烧】。',
            '对单体造成 [Int × 1.0] 的火焰伤害，施加1次【燃烧】。',
            '对单体造成 [Int × 1.1] 的火焰伤害，施加1次【燃烧】。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '炼狱吐息',
              effect: '对单体造成 [Int × 1.3] 的火焰伤害，施加2次【燃烧】。',
            },
            {
              id: 'B',
              name: '灼心之火',
              effect: '对单体造成 [Int × 1.1] 的火焰伤害，施加1次【燃烧】。有 [Cha × 5]% 概率施加1层【魅惑】。',
            },
          ],
        },
        {
          name: '心灵侵蚀',
          ap: 1,
          level: 1,
          attribute1: '心灵',
          attribute2: '远程',
          attribute3: '单体',
          effectByLevel: [
            '对单体造成 [Int × 0.6 + Cha × 0.3] 的心灵伤害。',
            '对单体造成 [Int × 0.7 + Cha × 0.3] 的心灵伤害。',
            '对单体造成 [Int × 0.8 + Cha × 0.6] 的心灵伤害。',
            '对单体造成 [Int × 0.9 + Cha × 0.6] 的心灵伤害。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '心碎',
              effect: '对单体造成 [Int × 0.9 + Cha × 0.4] 的心灵伤害。目标生命值低于40%时，伤害+50%。',
            },
            {
              id: 'B',
              name: '蛊惑',
              effect: '对单体造成 [Int × 0.9 + Cha × 0.4] 的心灵伤害。施加1层【魅惑】。',
            },
          ],
        },
        {
          id: '虚无放逐',
          name: '虚无放逐',
          ap: 2,
          level: 1,
          attribute1: '奥术',
          attribute2: '自身',
          attribute3: '单体',
          effectByLevel: [
            '相位转移至虚空维度。夜露立刻变为完全不可选中状态（无法被敌方攻击，同样无法接受友方增益和治疗），持续至下个回合开始。回归时，对敌方全体造成 [Int × 0.6] 的奥术伤害。',
            '相位转移至虚空维度。夜露立刻变为完全不可选中状态（无法被敌方攻击，同样无法接受友方增益和治疗），持续至下个回合开始。回归时，对敌方全体造成 [Int × 0.7] 的奥术伤害。',
            '相位转移至虚空维度。夜露立刻变为完全不可选中状态（无法被敌方攻击，同样无法接受友方增益和治疗），持续至下个回合开始。回归时，对敌方全体造成 [Int × 0.8] 的奥术伤害。',
            '相位转移至虚空维度。夜露立刻变为完全不可选中状态（无法被敌方攻击，同样无法接受友方增益和治疗），持续至下个回合开始。回归时，对敌方全体造成 [Int × 0.9] 的奥术伤害。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '心灵尖啸',
              effect:
                '相位转移至虚空维度。夜露立刻变为完全不可选中状态（无法被敌方攻击，同样无法接受友方增益和治疗），持续至下个回合开始。回归时，对敌方全体造成 [Int × 0.9] 的心灵伤害。',
            },
            {
              id: 'B',
              name: '魔焰爆发',
              effect:
                '相位转移至虚空维度。夜露立刻变为完全不可选中状态（无法被敌方攻击，同样无法接受友方增益和治疗），持续至下个回合开始。回归时，对敌方全体造成 [Int × 0.9] 的奥术伤害，并对所有敌人施加1次【燃烧】。',
            },
          ],
        },
        {
          name: '妖艳业火',
          ap: 2,
          level: 1,
          attribute1: '火焰',
          attribute2: '远程',
          attribute3: '群体',
          effectByLevel: [
            '释放妖艳的紫红色魔焰席卷敌阵。对敌方全体造成 [Int × 0.9] 的火焰伤害，并施加2次【燃烧】。',
            '释放妖艳的紫红色魔焰席卷敌阵。对敌方全体造成 [Int × 1.0] 的火焰伤害，并施加2次【燃烧】。',
            '释放妖艳的紫红色魔焰席卷敌阵。对敌方全体造成 [Int × 1.1] 的火焰伤害，并施加2次【燃烧】。',
            '释放妖艳的紫红色魔焰席卷敌阵。对敌方全体造成 [Int × 1.2] 的火焰伤害，并施加2次【燃烧】。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '炼狱',
              effect: '对敌方全体造成 [Int × 1.2] 的火焰伤害，施加2次【燃烧】，并立即结算一次燃烧伤害。',
            },
            {
              id: 'B',
              name: '魅焰',
              effect:
                '对敌方全体造成 [Int × 1.2] 的火焰伤害，对所有敌人依次判定，[Cha × 5]% 概率施加1次【魅惑】。并对所有在【魅惑】状态的敌人施加2次【燃烧】。',
            },
          ],
        },
      ],
      specialSkills: [
        {
          id: '魅魔之吻',
          name: '魅魔之吻',
          ap: 1,
          attribute1: '心灵',
          attribute2: '近战',
          attribute3: '单体',
          effect:
            '对单体造成 [Int × 0.8 + Cha × 0.8] 的心灵伤害，施加1层【魅惑】。若此攻击使目标【魅惑】层数达到3层，目标立即对自身造成 [目标ATK × 1.0] 的伤害。',
        },
        {
          id: '精神渗透',
          name: '精神渗透',
          ap: 0,
          attribute1: '被动',
          attribute2: '',
          attribute3: '',
          effect: '夜露造成的心灵伤害若被闪避，改为造成50%的伤害，但不附加任何额外效果。',
        },
        {
          id: '甜蜜支配',
          name: '甜蜜支配',
          ap: 0,
          attribute1: '被动',
          attribute2: '',
          attribute3: '',
          effect: '夜露对处于【魅惑】状态的敌人造成伤害时，有50%概率不消耗【魅惑】层数（仍然享受增伤效果）。',
        },
        {
          id: '灵魂盛宴',
          name: '灵魂盛宴',
          ap: 2,
          attribute1: '心灵',
          attribute2: '远程',
          attribute3: '群体',
          effect: '对敌方全体造成 [Cha × 1.2] 的心灵伤害。每个处于【魅惑】状态的敌人使夜露回复 [Cha × 1.0] 的生命值。',
        },
        {
          id: '竭魂之火',
          name: '竭魂之火',
          ap: 2,
          attribute1: '火焰',
          attribute2: '远程',
          attribute3: '单体',
          effect:
            '引爆目标体内的魔焰。对单体造成 [Int × 1.2] 的火焰伤害。消耗目标身上所有【燃烧】层数，造成双倍于消耗层数的伤害。',
        },
      ],
      specialSkillsUnlocked: [],
    },
    岚: {
      name: '岚',
      from: '幻界',
      introduce: '漫游枪侠·远程输出',
      level: 5,
      sta: 8,
      exp: 0,
      str: 10,
      agi: 14,
      int: 4,
      def: 4,
      luk: 7,
      cha: 9,
      bonusStr: 0,
      bonusAgi: 0,
      bonusInt: 0,
      bonusSta: 0,
      bonusDef: 0,
      passiveSkills: [
        {
          name: '猎戏',
          effect:
            '岚每次使用近战技能命中目标后，获得1层【锁定】；\n每次使用远程技能命中目标后，获得1层【扑杀】。\n每消耗1层【锁定】或【扑杀】，获得1层【心满意足】。',
        },
      ],
      skills: [
        {
          name: '攻击',
          ap: 1,
          effect: '对所选单位造成 [Str × 1] 点伤害',
          basic: true,
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '单体',
        },
        {
          name: '防御',
          ap: 1,
          effect: '对自身给予 [Def × 1] 点护盾，持续到自己的下个回合开始',
          basic: true,
          attribute1: '增益',
          attribute2: '自身',
          attribute3: '单体',
        },
        {
          name: '旋风踢',
          ap: 1,
          level: 1,
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '群体',
          effectByLevel: [
            '腾空旋转施展凌厉踢击。对敌方全体造成 [Str × 0.5] 的物理伤害。命中至少1个目标后，岚获得1层【灵巧】。',
            '腾空旋转施展凌厉踢击。对敌方全体造成 [Str × 0.55] 的物理伤害。命中至少1个目标后，岚获得1层【灵巧】。',
            '腾空旋转施展凌厉踢击。对敌方全体造成 [Str × 0.6] 的物理伤害。命中至少1个目标后，岚获得1层【灵巧】。',
            '腾空旋转施展凌厉踢击。对敌方全体造成 [Str × 0.65] 的物理伤害。命中至少1个目标后，岚获得1层【灵巧】。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '疾风踢',
              effect:
                '对敌方全体造成 [Str × 0.65] 的物理伤害。对主目标额外进行1次踢击判定，造成 [Agi × 0.4] 的物理伤害。命中后获得1层【灵巧】。',
            },
            {
              id: 'B',
              name: '击崩踢',
              effect: '对敌方全体造成 [Str × 0.65] 的物理伤害。对所有命中目标施加1层【迟缓】。命中后获得1层【灵巧】。',
            },
          ],
        },
        {
          name: '猫步',
          ap: 1,
          level: 1,
          attribute1: '增益',
          attribute2: '自身',
          attribute3: '单体',
          effectByLevel: [
            '进入灵动的闪避姿态。岚获得 [Def × 0.8] 的护盾，并获得1层【灵巧】。',
            '进入灵动的闪避姿态。岚获得 [Def × 1.0] 的护盾，并获得1层【灵巧】。',
            '进入灵动的闪避姿态。岚获得 [Def × 1.2] 的护盾，并获得1层【灵巧】。',
            '进入灵动的闪避姿态。岚获得 [Def × 1.4] 的护盾，并获得1层【灵巧】。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '影舞',
              effect:
                '岚获得 [Def × 1.4] 的护盾，并获得2层【灵巧】至下回合。成功闪避时对攻击者反击，造成 [Agi × 0.4] 的物理伤害。',
            },
            {
              id: 'B',
              name: '疾风',
              effect: '岚获得 [Def × 0.8 + Agi × 0.6] 的护盾，并获得1层【灵巧】和1层【敏捷强化】。',
            },
          ],
        },
        {
          name: '瞬星射击',
          ap: 2,
          level: 1,
          attribute1: '物理',
          attribute2: '远程',
          attribute3: '单体',
          effectByLevel: [
            '拔枪瞬射，对单体进行3次射击判定，每次造成 [Agi × 0.4] 的物理伤害。',
            '拔枪瞬射，对单体进行3次射击判定，每次造成 [Agi × 0.45] 的物理伤害。',
            '拔枪瞬射，对单体进行3次射击判定，每次造成 [Agi × 0.45] 的物理伤害。',
            '拔枪瞬射，对单体进行3次射击判定，每次造成 [Agi × 0.5] 的物理伤害。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '穿甲弹',
              effect:
                '对单体进行3次射击判定，每次造成 [Agi × 0.5] 的物理伤害。获得1层【精准】。首次命中施加1层【破甲】。',
            },
            {
              id: 'B',
              name: '爆裂弹',
              effect:
                '对单体进行3次射击判定，每次造成 [Agi × 0.5] 的物理伤害。获得2层【精准】。此技能暴击伤害提升至250%。',
            },
          ],
        },
        {
          name: '绞首射击',
          ap: 2,
          level: 1,
          attribute1: '物理',
          attribute2: '混合',
          attribute3: '单体',
          effectByLevel: [
            '双腿绞住目标颈部锁死，枪口抵住头部射击。对单体造成 [Str × 0.9] 的近战物理伤害（绞锁），随后进行1次射击判定，造成 [Agi × 0.9] 的远程物理伤害。',
            '双腿绞住目标颈部锁死，枪口抵住头部射击。对单体造成 [Str × 1.0] 的近战物理伤害（绞锁），随后进行1次射击判定，造成 [Agi × 1.0] 的远程物理伤害。',
            '双腿绞住目标颈部锁死，枪口抵住头部射击。对单体造成 [Str × 1.1] 的近战物理伤害（绞锁），随后进行1次射击判定，造成 [Agi × 1.1] 的远程物理伤害。',
            '双腿绞住目标颈部锁死，枪口抵住头部射击。对单体造成 [Str × 1.2] 的近战物理伤害（绞锁），随后进行1次射击判定，造成 [Agi × 1.2] 的远程物理伤害。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '处刑',
              effect:
                '双腿绞住目标颈部锁死，枪口抵住头部射击。对单体造成 [Str × 1.2] 的近战物理伤害（绞锁），随后进行1次射击判定，造成 [Agi × 1.2] 的远程物理伤害。射击伤害必定暴击。',
            },
            {
              id: 'B',
              name: '压制',
              effect:
                '双腿绞住目标颈部锁死，枪口抵住头部射击。对单体造成 [Str × 1.2] 的近战物理伤害（绞锁），施加2层【迟缓】和2层【迟钝】，随后进行1次射击判定，造成 [Agi × 1.2] 的远程物理伤害。',
            },
          ],
        },
      ],
      specialSkills: [
        {
          id: '蔷薇风暴',
          name: '蔷薇风暴',
          ap: 3,
          attribute1: '物理',
          attribute2: '远程',
          attribute3: '单体',
          effect: '对单体进行6次攻击判定，每次造成 [Agi × 0.5] 物理伤害。每次暴击使下一次判定的伤害+25%。',
        },
        {
          id: '浮空速射',
          name: '浮空速射',
          ap: 2,
          attribute1: '物理',
          attribute2: '远程',
          attribute3: '群体',
          effect:
            '跃入空中进行全方位扫射。对敌方全体进行2次射击判定，每次造成 [Agi × 0.4] 的物理伤害。施放后岚获得2层【灵巧】至下回合。若本回合此技能之前已使用过近战技能，改为造成 [Agi × 0.6] 的物理伤害。',
        },
        {
          id: '猎手本能',
          name: '猎手本能',
          ap: 0,
          attribute1: '被动',
          attribute2: '',
          attribute3: '',
          effect: '岚对生命值低于30%的敌人造成的伤害+25%。击杀敌人时，回复 [Agi × 0.5] 的生命值。',
        },
        {
          id: '死亡之眼',
          name: '死亡之眼',
          ap: 1,
          attribute1: '增益',
          attribute2: '自身',
          attribute3: '单体',
          effect:
            '岚锁定目标，获得2层【精准】和2层【专注】。岚的下一次命中的攻击额外对该目标施加1层【迟缓】和1次【流血】。',
        },
        {
          id: '弹跳踩踏',
          name: '弹跳踩踏',
          ap: 1,
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '单体',
          effect:
            '踩踏目标头部借力弹跳。对单体造成 [Str × 0.8] 的物理伤害，并使岚的下一次远程技能伤害+30%（与【扑杀】可叠加加算）。若目标处于【迟缓】状态，此技能伤害+50%。',
        },
      ],
      specialSkillsUnlocked: [],
    },
    清漓: {
      name: '清漓',
      from: '幻界',
      introduce: '醉太懒狗还没写',
      level: 5,
      sta: 8,
      exp: 0,
      str: 12,
      agi: 10,
      int: 6,
      def: 4,
      luk: 10,
      cha: 8,
      bonusStr: 0,
      bonusAgi: 0,
      bonusInt: 0,
      bonusSta: 0,
      bonusDef: 0,
      passiveSkills: [
        {
          name: '福泽',
          effect:
            '清漓的龙族血脉散发祥瑞之气。全体友方单位获得3的幸运。此外，清漓自身每拥有1的幸运，额外获得3%暴击率。',
        },
      ],
      skills: [
        {
          name: '攻击',
          ap: 1,
          effect: '对所选单位造成 [Str × 1] 点伤害',
          basic: true,
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '单体',
        },
        {
          name: '防御',
          ap: 1,
          effect: '对自身给予 [Def × 1] 点护盾，持续到自己的下个回合开始',
          basic: true,
          attribute1: '增益',
          attribute2: '自身',
          attribute3: '单体',
        },
        {
          name: '灵犀',
          ap: 1,
          level: 5,
          advancement: 'B',
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '单体',
          effectByLevel: [
            '以流水般的剑招攻击目标。对单体造成 [Str × 1.0] 的物理伤害。若暴击，额外追加1次 [Agi × 0.3] 的物理伤害。',
            '以流水般的剑招攻击目标。对单体造成 [Str × 1.1] 的物理伤害。若暴击，额外追加1次 [Agi × 0.3] 的物理伤害。',
            '以流水般的剑招攻击目标。对单体造成 [Str × 1.1] 的物理伤害。若暴击，额外追加1次 [Agi × 0.4] 的物理伤害。',
            '以流水般的剑招攻击目标。对单体造成 [Str × 1.2] 的物理伤害。若暴击，额外追加1次 [Agi × 0.4] 的物理伤害。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '破浪',
              effect:
                '以流水般的剑招攻击目标。对单体造成 [Str × 1.2] 的物理伤害，若暴击，额外追加1次 [Agi × 0.4] 的物理伤害。并施加1层【破甲】和1层【迟钝】。',
            },
            {
              id: 'B',
              name: '涟漪',
              effect:
                '以流水般的剑招攻击目标。对单体造成 [Str × 1.2] 的物理伤害，若暴击，额外追加1次 [Agi × 0.4] 的物理伤害。每次暴击获得1层【剑势】。',
            },
          ],
        },
        {
          name: '护卫',
          ap: 1,
          level: 5,
          advancement: 'B',
          attribute1: '增益',
          attribute2: '自身',
          attribute3: '单体',
          effectByLevel: [
            '操纵剑气环绕周身形成防护。清漓获得 [Def × 0.6 + Agi × 0.4] 的护盾。',
            '操纵剑气环绕周身形成防护。清漓获得 [Def × 0.7 + Agi × 0.4] 的护盾。',
            '操纵剑气环绕周身形成防护。清漓获得 [Def × 0.7 + Agi × 0.6] 的护盾。',
            '操纵剑气环绕周身形成防护。清漓获得 [Def × 0.8 + Agi × 0.6] 的护盾。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '叠浪',
              effect: '清漓获得 [Def × 0.8 + Agi × 0.6] 的护盾，并为受伤最重的友方提供50%效能的护盾。',
            },
            {
              id: 'B',
              name: '剑心',
              effect: '清漓获得 [Def × 0.8 + Agi × 0.6] 的护盾，并获得1层【剑势】和1层【精准】。',
            },
          ],
        },
        {
          name: '沧澜',
          ap: 2,
          level: 5,
          advancement: 'B',
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '单体',
          effectByLevel: [
            '使用基础【攻击】命中目标时获得1层【剑势】。\n消耗全部【剑势】，对单体造成 [(Str+Agi)×剑势层数×0.175] 的物理伤害。',
            '使用基础【攻击】命中目标时获得1层【剑势】。\n消耗全部【剑势】，对单体造成 [(Str+Agi)×剑势层数×0.2] 的物理伤害。',
            '使用基础【攻击】命中目标时获得1层【剑势】。\n消耗全部【剑势】，对单体造成 [(Str+Agi)×剑势层数×0.225] 的物理伤害。',
            '使用基础【攻击】命中目标时获得1层【剑势】。\n消耗全部【剑势】，对单体造成 [(Str+Agi)×剑势层数×0.25] 的物理伤害。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '流溯',
              effect:
                '使用基础【攻击】命中目标时获得1层【剑势】。\n消耗全部【剑势】，对单体造成 [(Str+Agi)×剑势层数×0.25] 的物理伤害。释放后，返还本次消耗剑势层数的40%。',
            },
            {
              id: 'B',
              name: '潮汐',
              effect:
                '使用基础【攻击】命中目标时获得1层【剑势】。\n回合开始时每有5点【智力】获得1层【剑势】；回合结束时自动消耗全部【剑势】，对正前方单体敌人造成 [(Str+Agi)×剑势层数×0.25] 的物理伤害（不消耗 AP）。',
            },
          ],
        },
        {
          name: '碧落',
          ap: 2,
          level: 5,
          advancement: 'B',
          locked: true,
          attribute1: '物理',
          attribute2: '远程',
          attribute3: '群体',
          effectByLevel: [
            '挥剑释放剑气横扫战场。对敌方全体造成 [Str × 0.5 + Agi × 0.3] 的物理伤害。每次暴击时，清漓获得1层【剑势】。',
            '挥剑释放剑气横扫战场。对敌方全体造成 [Str × 0.5 + Agi × 0.4] 的物理伤害。每次暴击时，清漓获得1层【剑势】。',
            '挥剑释放剑气横扫战场。对敌方全体造成 [Str × 0.6 + Agi × 0.4] 的物理伤害。每次暴击时，清漓获得1层【剑势】。',
            '挥剑释放剑气横扫战场。对敌方全体造成 [Str × 0.6 + Agi × 0.5] 的物理伤害。每次暴击时，清漓获得1层【剑势】。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '万剑归宗',
              effect: '对敌方全体造成 [Str × 0.6 + Agi × 0.5] 的物理伤害。每次命中时，清漓获得1层【剑势】。',
            },
            {
              id: 'B',
              name: '一剑霜寒',
              effect:
                '对敌方全体造成 [Str × 0.6 + Agi × 0.5] 的物理伤害，对所有命中目标施加1层【迟缓】。每次暴击时，清漓获得1层【剑势】。',
            },
          ],
        },
      ],
      specialSkills: [
        {
          id: '行云流水',
          name: '行云流水',
          ap: 0,
          attribute1: '被动',
          attribute2: '',
          attribute3: '',
          effect: '清漓每次执行的指令与上一次执行的指令类型不同时，获得1层【剑势】；否则失去1层【剑势】。',
        },
        {
          id: '踏浪行歌',
          name: '踏浪行歌',
          ap: 1,
          attribute1: '增益',
          attribute2: '自身',
          attribute3: '单体',
          effect:
            '清漓进入【踏浪】状态，持续至下回合开始。期间每使用一个消耗 AP 的指令后额外获得1层【剑势】。若本回合已使用不少于3种不同指令（不含踏浪行歌），本回合内【沧澜】伤害+30%。',
        },
        {
          id: '千里剑光',
          name: '千里剑光',
          ap: 0,
          attribute1: '被动',
          attribute2: '',
          attribute3: '',
          effect:
            '【沧澜】视为远程。沧澜命中后对目标施加2层【破甲】与2层【迟钝】；若消耗的【剑势】≥12，额外施加1层【虚弱】。若选择【沧澜·潮汐】，自动攻击优先同列后排存活目标。',
        },
        {
          id: '祥瑞庇佑',
          name: '祥瑞庇佑',
          ap: 1,
          attribute1: '增益',
          attribute2: '远程',
          attribute3: '群体',
          effect: '友方全体获得2层【精准】与1层【激励】；清漓额外获得2层【剑势】。',
        },
        {
          id: '剑心通明',
          name: '剑心通明',
          ap: 0,
          attribute1: '被动',
          attribute2: '',
          attribute3: '',
          effect: '【沧澜】的暴击伤害倍率提升至300%。回合开始时，若【剑势】层数≥10，清漓获得1层【激励】。',
        },
      ],
      specialSkillsUnlocked: ['行云流水', '踏浪行歌', '千里剑光', '祥瑞庇佑', '剑心通明'],
      buffs: [],
    },
    凌遥仙: {
      name: '凌遥仙',
      from: '幻界',
      introduce: '待填写',
      level: 1,
      exp: 0,
      str: 4,
      agi: 6,
      int: 16,
      sta: 8,
      def: 8,
      luk: 8,
      cha: 9,
      bonusStr: 0,
      bonusAgi: 0,
      bonusInt: 0,
      bonusSta: 0,
      bonusDef: 0,
      bonusLuk: 0,
      bonusCha: 0,
      passiveSkills: [
        {
          name: '星海咏叹',
          effect: '凌遥仙可以从技能中获得或使用【诗章】。',
        },
      ],
      skills: [
        {
          name: '攻击',
          ap: 1,
          effect: '对所选单位造成 [Str × 1] 点伤害',
          basic: true,
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '单体',
        },
        {
          name: '防御',
          ap: 1,
          effect: '对自身给予 [Def × 1] 点护盾，持续到自己的下个回合开始',
          basic: true,
          attribute1: '增益',
          attribute2: '自身',
          attribute3: '单体',
        },
        {
          name: '星语祝祷',
          ap: 2,
          level: 1,
          attribute1: '增益',
          attribute2: '远程',
          attribute3: '单体',
          effectByLevel: [
            '低声咏唱星辰祝词。为目标回复 [Int × 1.2] 的治疗效果，为友方单体施加2层【力量强化】、【敏捷强化】或【智力强化】，总会强化最高的那个属性。获得2层【诗章】。',
            '低声咏唱星辰祝词。为目标回复 [Int × 1.4] 的治疗效果，为友方单体施加2层【力量强化】、【敏捷强化】或【智力强化】，总会强化最高的那个属性。获得2层【诗章】。',
            '低声咏唱星辰祝词。为目标回复 [Int × 1.6] 的治疗效果，为友方单体施加2层【力量强化】、【敏捷强化】或【智力强化】，总会强化最高的那个属性。获得2层【诗章】。',
            '低声咏唱星辰祝词。为目标回复 [Int × 1.8] 的治疗效果，为友方单体施加2层【力量强化】、【敏捷强化】或【智力强化】，总会强化最高的那个属性。获得2层【诗章】。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '星辉灌注',
              effect:
                '低声咏唱星辰祝词。为目标回复 [Int × 1.8] 的治疗效果，为友方单体施加3层【力量强化】、【敏捷强化】或【智力强化】，总会强化最高的那个属性。获得2层【诗章】。',
            },
            {
              id: 'B',
              name: '星光共鸣',
              effect:
                '低声咏唱星辰祝词。为目标回复 [Int × 1.8] 的治疗效果，为友方单体施加2层【力量强化】、【敏捷强化】或【智力强化】，总会强化最高的那个属性。额外施加2层【防御强化】。获得2层【诗章】。',
            },
          ],
        },
        {
          name: '辉烬壁障',
          ap: 1,
          level: 1,
          attribute1: '增益',
          attribute2: '远程',
          attribute3: '单体',
          effectByLevel: [
            '凝聚星光编织防护屏障。为友方单体提供 [Int × 0.6 + Def × 0.4] 的护盾。获得2层【诗章】。',
            '凝聚星光编织防护屏障。为友方单体提供 [Int × 0.7 + Def × 0.4] 的护盾。获得2层【诗章】。',
            '凝聚星光编织防护屏障。为友方单体提供 [Int × 0.7 + Def × 0.5] 的护盾。获得2层【诗章】。',
            '凝聚星光编织防护屏障。为友方单体提供 [Int × 0.8 + Def × 0.5] 的护盾。获得2层【诗章】。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '星纱帷幕',
              effect:
                '凝聚星光编织防护屏障。为友方单体提供 [Int × 0.9 + Def × 0.5] 的护盾。获得2层【诗章】。若当前【诗章】≥5层，额外清除目标身上1个负面状态。施加1层【扰魔】。',
            },
            {
              id: 'B',
              name: '辉烬坚壁',
              effect:
                '凝聚星光编织防护屏障。为友方单体提供 [Int × 1.2 + Def × 0.6] 的护盾。获得2层【诗章】。护盾消失时，将剩余护盾值的50%转化为对目标的治疗。施加1层【格挡】。',
            },
          ],
        },
        {
          name: '星海调和',
          ap: 1,
          level: 1,
          attribute1: '治疗',
          attribute2: '远程',
          attribute3: '单体',
          effectByLevel: [
            '为友方单体施加2层【再生】。获得1层【诗章】。',
            '为友方单体施加2层【再生】。回复 [Int × 0.2] 生命值。获得1层【诗章】。',
            '为友方单体施加2层【再生】。回复 [Int × 0.4] 生命值。获得1层【诗章】。',
            '为友方单体施加2层【再生】。回复 [Int × 0.6] 生命值。获得1层【诗章】。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '星海漫灌',
              effect:
                '施加2层【再生】，回复 [Int × 0.6] 生命值。获得1层【诗章】。若目标HP低于50%，消耗2层【诗章】使本次治疗效果翻倍。',
            },
            {
              id: 'B',
              name: '星辉回流',
              effect:
                '施加2层【再生】，回复 [Int × 0.6] 生命值。获得1层【诗章】。消耗2层【诗章】额外对其施加 [Int × 0.6] 的护盾。',
            },
          ],
        },
        {
          name: '天穹颂歌',
          ap: 2,
          level: 1,
          attribute1: '增益',
          attribute2: '远程',
          attribute3: '群体',
          effectByLevel: [
            '为友方全体施加 [Int × 0.75] 的护盾。如果诗章层数≥5，消耗5层诗章并为友方全体施加1层【激励】。',
            '为友方全体施加 [Int × 0.8] 的护盾。如果诗章层数≥5，消耗5层诗章并为友方全体施加1层【激励】。',
            '为友方全体施加 [Int × 0.85] 的护盾。如果诗章层数≥5，消耗5层诗章并为友方全体施加1层【激励】。',
            '为友方全体施加 [Int × 0.9] 的护盾。如果诗章层数≥5，消耗5层诗章并为友方全体施加1层【激励】。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '终末咏唱',
              effect:
                '为友方全体施加 [Int × 0.9] 的护盾。如果诗章层数≥5，消耗5层诗章并为友方全体施加1层【激励】，并对敌方全体施加1层【脆弱】。',
            },
            {
              id: 'B',
              name: '星河永续',
              effect:
                '为友方全体施加 [Int × 0.9] 的护盾。如果诗章层数≥5，消耗5层诗章并为友方全体施加1层【激励】和1层【再生】。',
            },
          ],
        },
      ],
      specialSkills: [
        {
          id: '星辰定锚',
          name: '星辰定锚',
          ap: 2,
          attribute1: '减益',
          attribute2: '远程',
          attribute3: '单体',
          effect: '对指定目标进行3次命中判定，每次判定成功对其施加1层【眩晕】。获得3层【诗章】。每场战斗限1次。',
        },
        {
          id: '星辰加速',
          name: '星辰加速',
          ap: 1,
          attribute1: '增益',
          attribute2: '远程',
          attribute3: '单体',
          effect: '为指定友军施加3层【星辰加速】：本回合内下3次行动的技能 AP 消耗-1（至少为1点）。',
        },
        {
          id: '星命逆转',
          name: '星命逆转',
          ap: 2,
          attribute1: '增益',
          attribute2: '远程',
          attribute3: '单体',
          effect:
            '消耗5层【诗章】，为指定友方施加【星命】至战斗结束：受到致死伤害时保留1点生命并获得护盾，随后移除【星命】。每场战斗限1次。',
        },
        {
          id: '命仪精准',
          name: '命仪精准',
          ap: 2,
          attribute1: '增益',
          attribute2: '远程',
          attribute3: '单体',
          effect:
            '为指定友军施加1层【命仪精准】：本回合内下一次攻击必定命中且必定暴击；若该次攻击击杀敌人，凌遥仙回复1点AP。每场战斗限1次。',
        },
        {
          id: '永恒咏唱',
          name: '永恒咏唱',
          ap: 0,
          attribute1: '被动',
          attribute2: '',
          attribute3: '',
          effect: '凌遥仙为友方施加的所有【强化】类效果以同等层数转化为对应的【增幅】效果。',
        },
      ],
      specialSkillsUnlocked: ['永恒咏唱', '星辰定锚', '星辰加速', '星命逆转', '命仪精准'],
      buffs: [],
    },
    月见遥: {
      name: '月见遥',
      from: '幻界',
      introduce: '待填写',
      level: 5,
      exp: 0,
      str: 4,
      agi: 10,
      int: 14,
      sta: 8,
      def: 4,
      luk: 6,
      cha: 9,
      bonusStr: 0,
      bonusAgi: 0,
      bonusInt: 0,
      bonusSta: 0,
      bonusDef: 0,
      bonusLuk: 0,
      bonusCha: 0,
      passiveSkills: [
        {
          name: '观测者的愉悦',
          effect:
            '场上每有一次任意单位攻击判定未命中，或攻击方带有【混乱】且该次攻击命中时，月见遥获得1层【愉悦】。',
        },
      ],
      skills: [
        {
          name: '攻击',
          ap: 1,
          effect: '对所选单位造成 [Str × 1] 点伤害',
          basic: true,
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '单体',
        },
        {
          name: '防御',
          ap: 1,
          effect: '对自身给予 [Def × 1] 点护盾，持续到自己的下个回合开始',
          basic: true,
          attribute1: '增益',
          attribute2: '自身',
          attribute3: '单体',
        },
        {
          name: '镜花水月',
          ap: 1,
          level: 5,
          advancement: 'A',
          attribute1: '心灵',
          attribute2: '远程',
          attribute3: '单体',
          effectByLevel: [
            '对敌方制造精神幻觉。对单体造成 [Int × 0.7] 的心灵伤害。若目标处于控制状态，伤害+20%。',
            '对敌方制造精神幻觉。对单体造成 [Int × 0.75] 的心灵伤害。若目标处于控制状态，伤害+20%。',
            '对敌方制造精神幻觉。对单体造成 [Int × 0.75] 的心灵伤害。若目标处于控制状态，伤害+25%。',
            '对敌方制造精神幻觉。对单体造成 [Int × 0.8] 的心灵伤害。若目标处于控制状态，伤害+25%。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '心碎镜像',
              effect:
                '对敌方制造精神幻觉。对单体造成 [Int × 0.8] 的心灵伤害。若目标处于控制状态，伤害+50%，并回复月见遥 [Int × 0.3] 的生命值。',
            },
            {
              id: 'B',
              name: '幻影穿刺',
              effect:
                '对敌方制造精神幻觉。对单体造成 [Int × 0.8] 的心灵伤害。若目标有【魅惑】，此攻击必定命中，并施加1层【碎魔】。',
            },
          ],
        },
        {
          name: '虚妄护盾',
          ap: 1,
          level: 5,
          advancement: 'A',
          attribute1: '增益',
          attribute2: '远程',
          attribute3: '单体',
          effectByLevel: [
            '编织幻术护盾。获得一层特殊护盾，护盾值 = [Int × 0.5 + Def × 0.5]。该护盾只能承受1次攻击。',
            '编织幻术护盾。获得一层特殊护盾，护盾值 = [Int × 0.6 + Def × 0.6]。该护盾只能承受1次攻击。',
            '编织幻术护盾。获得一层特殊护盾，护盾值 = [Int × 0.7 + Def × 0.7]。该护盾只能承受1次攻击。',
            '编织幻术护盾。获得一层特殊护盾，护盾值 = [Int × 0.8 + Def × 0.8]。该护盾只能承受1次攻击。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '镜面反射',
              effect:
                '编织幻术护盾。获得一层特殊护盾，护盾值 = [Int × 0.8 + Def × 0.8]。该护盾只能承受1次攻击。护盾破碎时，将受到伤害的50%反弹给攻击者。',
            },
            {
              id: 'B',
              name: '幻影替身',
              effect:
                '编织幻术护盾。对任意友方施加一层特殊护盾，护盾值 = [Int × 0.8 + Def × 0.8]。该护盾只能承受1次攻击。',
            },
          ],
        },
        {
          name: '迷雾幻境',
          ap: 2,
          level: 5,
          advancement: 'A',
          attribute1: '减益',
          attribute2: '远程',
          attribute3: '群体',
          effectByLevel: [
            '释放扭曲视觉的幻雾笼罩战场。对敌方全体施加2层【恍惚】。',
            '释放扭曲视觉的幻雾笼罩战场。对敌方全体施加2层【恍惚】。',
            '释放扭曲视觉的幻雾笼罩战场。对敌方全体施加3层【恍惚】。',
            '释放扭曲视觉的幻雾笼罩战场。对敌方全体施加3层【恍惚】。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '虚实倒错',
              effect:
                '释放扭曲视觉的幻雾笼罩战场。对敌方全体施加3层【恍惚】。敌方每次Miss时，对自身造成 [Atk × 0.3] 的魔法伤害。',
            },
            {
              id: 'B',
              name: '认知崩解',
              effect:
                '释放扭曲视觉的幻雾笼罩战场。对敌方全体施加3层【恍惚】和3层【迟钝】。',
            },
          ],
        },
        {
          name: '心智侵蚀',
          ap: 1,
          level: 5,
          advancement: 'A',
          attribute1: '心灵',
          attribute2: '远程',
          attribute3: '单体',
          effectByLevel: [
            '将幻术直接注入目标心智。对单体造成 [Int × 0.6] 的心灵伤害，并有50%概率施加1层【碎魔】。',
            '将幻术直接注入目标心智。对单体造成 [Int × 0.7] 的心灵伤害，并有50%概率施加1层【碎魔】。',
            '将幻术直接注入目标心智。对单体造成 [Int × 0.8] 的心灵伤害，施加1层【碎魔】。',
            '将幻术直接注入目标心智。对单体造成 [Int × 0.9] 的心灵伤害，施加1层【碎魔】。',
          ],
          advancementReplacesEffect: true,
          advancementOptions: [
            {
              id: 'A',
              name: '断筋蚀骨',
              effect:
                '将幻术直接注入目标心智。对单体造成 [Int × 0.9] 的心灵伤害，施加1层【碎魔】。额外有 [Cha × 5]% 概率施加1层【缴械】。',
            },
            {
              id: 'B',
              name: '封喉噤声',
              effect:
                '将幻术直接注入目标心智。对单体造成 [Int × 0.9] 的心灵伤害，施加1层【碎魔】。额外有 [Cha × 5]% 概率施加1层【沉默】。',
            },
          ],
        },
      ],
      specialSkills: [
        {
          id: '大妃的魔宴',
          name: '大妃的魔宴',
          ap: 3,
          attribute1: '奥术',
          attribute2: '远程',
          attribute3: '单体',
          effect:
            '对敌方单体造成 [Int × 2.5] 的奥术伤害。目标身上每有1种不同的减益，伤害+15%（最高+75%）。',
        },
        {
          id: '傀儡剧场',
          name: '傀儡剧场',
          ap: 2,
          attribute1: '心灵',
          attribute2: '远程',
          attribute3: '群体',
          effect:
            '对敌方全体造成 [Int × 0.6] 的心灵伤害。随后每名敌方立即对除月见遥以外的随机单位发动一次攻击。',
        },
        {
          id: '完美谎言',
          name: '完美谎言',
          ap: 1,
          attribute1: '魔法',
          attribute2: '远程',
          attribute3: '群体',
          effect:
            '选择一名友方作为幻术替身。敌方下一次随机单体攻击若选中月见遥，则改为攻击该替身（仅生效一次）。',
        },
        {
          id: '心灵震爆',
          name: '心灵震爆',
          ap: 0,
          attribute1: '被动',
          attribute2: '',
          attribute3: '',
          effect:
            '当【愉悦】达到10层时自动触发：对敌方全体造成 [Int × 1.5] 的心灵伤害并清空【愉悦】。每场战斗最多2次。',
        },
        {
          id: '虚实颠倒',
          name: '虚实颠倒',
          ap: 2,
          attribute1: '魔法',
          attribute2: '远程',
          attribute3: '单体',
          effect:
            '对单体施加【虚实颠倒】1回合：受到的治疗效果转化为等量伤害，受到的HP伤害转化为等量治疗。',
        },
      ],
      specialSkillsUnlocked: [
        '大妃的魔宴',
        '傀儡剧场',
        '完美谎言',
        '心灵震爆',
        '虚实颠倒',
      ],
      buffs: [],
    },
  };

  /** 默认队伍顺序：按槽位 1～6 填角色名，空位用 null */
  window.DEFAULT_PARTY_ORDER = ['凌遥仙', '月见遥', null, null, '达芙妮', '丝伊德·白'];

  function getSpecialSkillsForChar(ch) {
    if (!ch) return [];
    return ch.specialSkills && Array.isArray(ch.specialSkills) ? ch.specialSkills : [];
  }
  /** 白牙召唤数据工厂：需传入 getDisplayStat(owner, key) 以计算属性。达芙妮使用「白牙！」后召唤，MAXHp=[Sta×8]，攻击=[Str×0.8]，防御=[Def×0.4+Str×0.4]，行动 2。 */
  function createSummonBaiya(getDisplayStat) {
    return {
      name: '白牙',
      ap: 2,
      maxHpFromOwner: function (owner) {
        return Math.floor((getDisplayStat(owner, 'sta') || 0) * 8);
      },
      atkFromOwner: function (owner) {
        return Math.floor((getDisplayStat(owner, 'str') || 0) * 0.8);
      },
      defFromOwner: function (owner) {
        return Math.floor((getDisplayStat(owner, 'def') || 0) * 0.4 + (getDisplayStat(owner, 'str') || 0) * 0.4);
      },
      skills: [
        {
          name: '横扫',
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '群体',
          ap: 1,
          effect: '挥舞利爪横扫。对敌方同排造成 [Atk × 0.6] 的物理伤害。',
        },
        {
          name: '撕咬',
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '单体',
          ap: 1,
          effect: '撕咬目标。对敌方单体造成 [Atk × 1.0] 的物理伤害，并施加1次【重伤】。',
        },
      ],
    };
  }
  function getBaiyaStatsFromOwner(owner, getDisplayStat) {
    if (!owner || !getDisplayStat) return { maxHp: 0, atk: 0, def: 0 };
    const summon = createSummonBaiya(getDisplayStat);
    return {
      maxHp: summon.maxHpFromOwner(owner),
      atk: summon.atkFromOwner(owner),
      def: summon.defFromOwner(owner),
    };
  }
  /**
   * 莉莉姆（史莱姆娘）召唤物数据工厂：属性随主人丝伊德·白。
   * MaxHP=[Sta×6]，ATK=[Int×0.5]，DEF=[Def×0.8+Sta×0.3]，AP=2。
   * 主动技「黏液包裹」「弹性护盾」在 battle.js executePlayer 中结算；被动 trait柔软躯体 在 resolveAttack 中已生效。
   */
  function createSummonLilim(getDisplayStat) {
    return {
      name: '莉莉姆',
      introduce: '史莱姆娘',
      ap: 2,
      daughterUnit: true,
      trait柔软躯体: true,
      passiveSkills: [
        {
          name: '柔软躯体',
          effect: '莉莉姆受到的物理伤害-30%，受到的魔法伤害+20%。',
        },
      ],
      maxHpFromOwner: function (owner) {
        return Math.floor((getDisplayStat(owner, 'sta') || 0) * 6);
      },
      atkFromOwner: function (owner) {
        return Math.floor((getDisplayStat(owner, 'int') || 0) * 0.5);
      },
      defFromOwner: function (owner) {
        return Math.floor(
          (getDisplayStat(owner, 'def') || 0) * 0.8 + (getDisplayStat(owner, 'sta') || 0) * 0.3,
        );
      },
      skills: [
        {
          name: '黏液包裹',
          ap: 1,
          attribute1: '减益',
          attribute2: '近战',
          attribute3: '单体',
          effect: '对单体造成 [ATK × 0.6] 的自然伤害，施加1层【迟缓】。',
        },
        {
          name: '弹性护盾',
          ap: 1,
          attribute1: '增益',
          attribute2: '远程',
          attribute3: '单体',
          effect: '为自身提供 [DEF × 1.2] 的护盾。',
        },
      ],
    };
  }
  function getLilimStatsFromOwner(owner, getDisplayStat) {
    if (!owner || !getDisplayStat) return { maxHp: 0, atk: 0, def: 0 };
    const summon = createSummonLilim(getDisplayStat);
    return {
      maxHp: summon.maxHpFromOwner(owner),
      atk: summon.atkFromOwner(owner),
      def: summon.defFromOwner(owner),
    };
  }
  /**
   * 可瑞（血触虫）召唤物数据：输出/破甲/持续伤害，属性随主人丝伊德·白。
   * MaxHP=[Sta×4]，ATK=[Str×0.6+Int×0.4]，DEF=[Def×0.3]，AP=2。
   * 主动技在 battle.js 通用攻击链 + 缠绕撕咬/血触侵蚀 debuff 分支中结算。
   */
  function createSummonKerui(getDisplayStat) {
    return {
      name: '可瑞',
      introduce: '血触虫',
      ap: 2,
      daughterUnit: true,
      maxHpFromOwner: function (owner) {
        return Math.floor((getDisplayStat(owner, 'sta') || 0) * 4);
      },
      atkFromOwner: function (owner) {
        const strV = getDisplayStat(owner, 'str') || 0;
        const intV = getDisplayStat(owner, 'int') || 0;
        return Math.floor(strV * 0.6 + intV * 0.4);
      },
      defFromOwner: function (owner) {
        return Math.floor((getDisplayStat(owner, 'def') || 0) * 0.3);
      },
      skills: [
        {
          name: '缠绕撕咬',
          ap: 1,
          attribute1: '物理',
          attribute2: '近战',
          attribute3: '单体',
          effect: '对单体造成 [ATK × 1.0] 的物理伤害，施加1层【破甲】。',
        },
        {
          name: '血触侵蚀',
          ap: 1,
          attribute1: '自然',
          attribute2: '近战',
          attribute3: '单体',
          effect: '对单体造成 [ATK × 0.7] 的自然伤害，施加2次【中毒】。',
        },
      ],
    };
  }
  function getKeruiStatsFromOwner(owner, getDisplayStat) {
    if (!owner || !getDisplayStat) return { maxHp: 0, atk: 0, def: 0 };
    const summon = createSummonKerui(getDisplayStat);
    return {
      maxHp: summon.maxHpFromOwner(owner),
      atk: summon.atkFromOwner(owner),
      def: summon.defFromOwner(owner),
    };
  }
  /**
   * 蒙特卡洛（阿娜温）召唤物数据：辅助/控制/范围削弱，属性随主人丝伊德·白。
   * MaxHP=[Sta×5]，ATK=[Int×0.7]，DEF=[Def×0.5]，AP=2。
   * 金粉弥漫/芬芳治愈在 battle.js 专用 executePlayer 中结算。
   */
  function createSummonMonteCarlo(getDisplayStat) {
    return {
      name: '蒙特卡洛',
      introduce: '阿娜温',
      ap: 2,
      daughterUnit: true,
      maxHpFromOwner: function (owner) {
        return Math.floor((getDisplayStat(owner, 'sta') || 0) * 5);
      },
      atkFromOwner: function (owner) {
        return Math.floor((getDisplayStat(owner, 'int') || 0) * 0.7);
      },
      defFromOwner: function (owner) {
        return Math.floor((getDisplayStat(owner, 'def') || 0) * 0.5);
      },
      skills: [
        {
          name: '金粉弥漫',
          ap: 1,
          attribute1: '减益',
          attribute2: '远程',
          attribute3: '群体',
          effect: '对敌方全体施加1层【恍惚】和1层【迟钝】。',
        },
        {
          name: '芬芳治愈',
          ap: 1,
          attribute1: '治疗',
          attribute2: '远程',
          attribute3: '单体',
          effect:
            '为目标回复 [ATK × 0.8] 的生命值，如果目标是丝伊德·白，并使其获得1层【孕育】。',
        },
      ],
    };
  }
  function getMonteCarloStatsFromOwner(owner, getDisplayStat) {
    if (!owner || !getDisplayStat) return { maxHp: 0, atk: 0, def: 0 };
    const summon = createSummonMonteCarlo(getDisplayStat);
    return {
      maxHp: summon.maxHpFromOwner(owner),
      atk: summon.atkFromOwner(owner),
      def: summon.defFromOwner(owner),
    };
  }
  /**
   * 堇（菌菇娘）召唤物数据：辅助/DOT/群体削弱，属性随主人丝伊德·白。
   * MaxHP=[Sta×5]，ATK=[Int×0.6]，DEF=[Def×0.4]，AP=2。
   * 战斗内释放逻辑待接入召唤技能时实现。
   */
  function createSummonJin(getDisplayStat) {
    return {
      name: '堇',
      introduce: '菌菇娘',
      ap: 2,
      daughterUnit: true,
      maxHpFromOwner: function (owner) {
        return Math.floor((getDisplayStat(owner, 'sta') || 0) * 5);
      },
      atkFromOwner: function (owner) {
        return Math.floor((getDisplayStat(owner, 'int') || 0) * 0.6);
      },
      defFromOwner: function (owner) {
        return Math.floor((getDisplayStat(owner, 'def') || 0) * 0.4);
      },
      skills: [
        {
          name: '孢子云',
          ap: 1,
          attribute1: '自然',
          attribute2: '远程',
          attribute3: '群体',
          effect:
            '对敌方全体造成 [ATK × 0.4] 的自然伤害，对所有命中目标施加1次【中毒】。',
        },
        {
          name: '再生菌丝',
          ap: 1,
          attribute1: '治疗',
          attribute2: '远程',
          attribute3: '单体',
          effect:
            '为目标施加2次【再生】，如果目标是丝伊德·白，并使其获得1层【孕育】。',
        },
      ],
    };
  }
  function getJinStatsFromOwner(owner, getDisplayStat) {
    if (!owner || !getDisplayStat) return { maxHp: 0, atk: 0, def: 0 };
    const summon = createSummonJin(getDisplayStat);
    return {
      maxHp: summon.maxHpFromOwner(owner),
      atk: summon.atkFromOwner(owner),
      def: summon.defFromOwner(owner),
    };
  }
  /** 从技能的三个属性字段（attribute1/2/3）拼出显示用字符串，如「物理/近战/群体」；若无则退回 tags */
  function getSkillTagsString(skill) {
    if (!skill) return '';
    const a1 = skill.attribute1 != null ? String(skill.attribute1).trim() : '';
    const a2 = skill.attribute2 != null ? String(skill.attribute2).trim() : '';
    const a3 = skill.attribute3 != null ? String(skill.attribute3).trim() : '';
    if (a1 || a2 || a3) return [a1, a2, a3].filter(Boolean).join('/');
    return skill.tags != null ? String(skill.tags) : '';
  }

  window.色色地牢_character = {
    getSpecialSkillsForChar: getSpecialSkillsForChar,
    createSummonBaiya: createSummonBaiya,
    getBaiyaStatsFromOwner: getBaiyaStatsFromOwner,
    createSummonLilim: createSummonLilim,
    getLilimStatsFromOwner: getLilimStatsFromOwner,
    createSummonKerui: createSummonKerui,
    getKeruiStatsFromOwner: getKeruiStatsFromOwner,
    createSummonMonteCarlo: createSummonMonteCarlo,
    getMonteCarloStatsFromOwner: getMonteCarloStatsFromOwner,
    createSummonJin: createSummonJin,
    getJinStatsFromOwner: getJinStatsFromOwner,
    getSkillTagsString: getSkillTagsString,
  };
})();
